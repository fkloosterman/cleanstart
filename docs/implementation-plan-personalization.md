...
- **Open decisions are collected in the register at the end.** A WP
  whose row says _blocked by D-n_ must not start until that decision
...
- **Migrations, two-stage:** a migration WP merging to `dev` is
  applied to the _dev_ database (freely — it's disposable). The
  _production_ database is touched only at promotion, by one person,
...
- **Tags at promotions only.** Each promotion merge gets an annotated
  ...
  needs. WP merges are _not_ tagged: their squash-merged PRs already
...
### WP0.1 Dev environment: database, branch, deployment — _D13: resolved, proceed_
...
model (see _Branching & release model_): create the protected `dev`
...
### WP0.2 Test infrastructure — _blocked by D15 (recommendation given)_
...
### WP0.3 CI pipeline — _depends WP0.2; blocked by D16 for the eval leg_
...
### WP0.4 Per-purpose model map — _D12: resolved, proceed_
...
### WP0.5 Eval harness scaffolding — _depends WP0.2, WP0.4_
...
### WP1.2 Migration #1 — _depends WP1.1 shape freeze; ideally after WP0.1_
...
### WP1.3 Upfront slots: stepper → profile — _blocked by D3_
...
### WP1.4 Profile extractor + eval fixtures — _depends WP1.1, WP0.4, WP0.5_
...
### WP1.5 Wire extraction into both chat endpoints — _depends WP1.3, WP1.4_
...
### WP1.6 Profile sidebar — _blocked by D1; depends WP1.5_
...
### WP1.8 Quick replies (`ask_user` tool) — _depends WP1.2, WP1.5_
...
### WP1.9 Guest→signup migration function — _depends WP1.5_
...
### WP1.10 Durable propagation — _blocked by D2; depends WP1.9_
...
readiness function. Playbook _copy_ (question wording, tone lines) is
...
### WP2.2 Context builder — _depends WP2.1_
...
### WP2.3 Extractor learns motivation — _depends WP2.1, WP1.4_
...
and do _not_ move on silence.

### WP2.4 Preset patches (interim) — _depends WP1.5_
...
_The content workstream (C1–C4, below) must be underway before WP3.2
has anything real to sync._
...
### WP3.2 Content tables + deploy-time sync — _depends WP3.1; migration #2_
...
### WP3.3 Candidate filtering + scoring — _depends WP3.2, WP2.1_
...
### WP3.3b Corpus coverage survey tool — _depends WP3.1, WP3.3_
...
### WP3.4 Chat grounding + `show_figure` + citations — _blocked by D4; depends WP3.3, WP2.2_
...
### WP3.5 Report document type + renderer — _blocked by D5; depends WP1.1_
...
### WP3.6 Composer + validation pipeline + evals — _depends WP3.3, WP3.5, WP0.5, and C2 corpus readiness; migration #2 includes `reports.document`_
...
is thin _as determined by code_ (`authoring_allowed_for`), rendered
...
the rest don't) — see _Delivery tiers_, seam 1.
...
### WP3.7 Lazy personalization — _depends WP3.6_
...
### WP3.8 Figure templates — _depends WP3.5_
...
### WP3.9 Export projections — _blocked by D18; depends WP3.5_
...
### WP3.10 Guest storage + rate limiting — _D14, D7: resolved; depends WP3.6, WP1.9_
...
### WP4.1 Items model — migration #3 — _depends WP3.6_
...
### WP4.2 "Your plan" panel (visit-based triggers) — _depends WP4.1_
...
### WP4.3 Follow-up sessions — _blocked by D8; depends WP4.1, WP2.2_
...
### WP4.4 Item state transitions + addenda — _depends WP4.3_
...
### WP4.5 Reveal mechanics — _depends WP4.4_
...
### WP4.6 Content-based triggers — _blocked by D6; depends WP4.1_
...
### WP4.7 Time-based reminders + email — _blocked by D17; depends WP4.6_
...
WP3.6 is a _cutover_ (report generation changes behavior for
...
Phases are ordered by _dependency_; tiers cut across them by _value_.
...
progressive disclosure is _static_: the top 2–3 render, held-back
...
- **D2 — "Session end" moment.** _(Open — Tier B.)_ The app has no
...
- **D4 — Citation rendering in chat.** ✅ **Resolved: compact
  ...
  the old "no URLs" prompt rule existed to stop model-_generated_
...
- **D6 — Notification surface.** _(Open — Tier C.)_ Where do
...
- **D7 — Guest limits.** ✅ **Resolved: moderate defaults — 40
  ...
  deploy-code change.**
- **D8 — Follow-up entry UX.** _(Open — Tier B.)_ Item card
...
- **D10 — Media licensing policy.** _(Open — needed before media
  lands in C2; components can ship without media.)_ What licenses are
...
- **D12 — Model policy.** ✅ **Resolved: a two-environment policy.**
  _Development:_ free OpenRouter models via the per-purpose model map
  ...
  _Production structured tasks (extraction, composition):_ a cheap
  ...
  DeepSeek V4 Flash, Haiku 4.5, a Flash-class model, _and_ the best
...
- **D16 — Evals in CI.** ✅ **Resolved: manual, with a required PR
  ...
  is known.
- **D17 — Email stack.** _(Open — Tier C.)_ Provider (e.g. Resend),
...
- **D18 — Figures in exports.** _(Open — Tier C; Tier A's WP3.9 ships
  the recommended minimal form: no figures, with an "available in the
  web version" note.)_ Rasterization approach decided if/when export
  usage justifies it.
- **D19 — Zip lookup dependency.** _(Open — Tier B; doesn't block
  WP1.3.)_ **[rec: bundle a static zip-prefix→state dataset,
